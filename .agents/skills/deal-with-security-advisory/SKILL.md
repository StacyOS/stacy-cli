...
```
gh api repos/stacy/skills/security-advisories/{{ghsaId}}
...
```
gh api --method POST \
  repos/stacy/skills/security-advisories/{{ghsaId}}/forks
...
```
gh api --method POST \
  repos/stacy/skills/security-advisories/{{ghsaId}}/cve
...
```
gh api repos/stacy/skills/security-advisories/{{ghsaId}} --jq '.credits'
...
```
gh api --method PATCH \
  repos/stacy/skills/security-advisories/{{ghsaId}} \
  --input - << 'EOF'
{
  "credits": [
    {
...
### Advisory
https://github.com/arpanstacy/stacy/security/advisories/{{ghsaId}}
...
gh release view v{{patchedVersion}} --repo arpanstacy/stacy
...
* The published advisory URL: `https://github.com/arpanstacy/stacy/security/advisories/{{ghsaId}}`
* The release URL
