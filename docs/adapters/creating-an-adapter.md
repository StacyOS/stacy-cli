...
Build a custom adapter to connect Stacy to any agent runtime.
...
|---|---|---|
| Source | Inside `stacy-fork` | Separate npm package |
| Distribution | Ships with Stacy | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Stacy's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@arpanstacy/stacy-adapter-utils/server-utils`
2. Build environment with `buildStacyEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@arpanstacy/stacy-adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildStacyEnv(agent)` | `@arpanstacy/stacy-adapter-utils/server-utils` | Inject `STACY_*` env vars |
| `renderTemplate(tpl, data)` | `@arpanstacy/stacy-adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@arpanstacy/stacy-adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@arpanstacy/stacy-adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Stacy's source:
...
`format-event.ts` — pretty-prints stdout for `stacy run --watch` using `picocolors`.
...
With these flags set, the Stacy UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Stacy source changes required.
...
Make Stacy skills discoverable to your agent runtime without writing to the agent's working directory:
