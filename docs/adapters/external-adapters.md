...
summary: Build, package, and distribute adapters as plugins without modifying Stacy source
---

Stacy supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Stacy's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `stacy-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Stacy | Published to npm or linked via `file:` |
| Updates | Requires Stacy release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@arpanstacy/stacy-adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `stacy.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@arpanstacy/stacy-adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Stacy vars injected
  const env = buildStacyEnv(agent);
...
#### Available Helpers from `@arpanstacy/stacy-adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildStacyEnv(agent)` | Inject `STACY_*` environment variables |
...
```sh
# Via the Stacy UI
# Settings → Adapters → Install from npm → "my-stacy-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-stacy-adapter"}'
```
...
Local adapters are symlinked into Stacy's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.stacy/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-stacy-adapter",
...
```ts
import type { AdapterSessionCodec } from "@arpanstacy/stacy-adapter-utils";
...
Other Stacy users can then install your adapter by package name from the UI or API.
