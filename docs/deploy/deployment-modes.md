...
Stacy supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm stacy allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Stacy emits a one-time claim URL at startup:
...
```sh
STACY_DEPLOYMENT_MODE=authenticated STACY_BIND=lan pnpm stacy run
```
