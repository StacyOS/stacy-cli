...
summary: Deploy Stacy to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy Stacy to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
export STACY_DOMAIN=stacy.example.com   # your domain
...
# Build
docker build -t stacy-server .

# Tag and push
docker tag stacy-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/stacy-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name stacy-alb \
  --description "Stacy ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name stacy-ecs \
  --description "Stacy ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name stacy-rds \
  --description "Stacy RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name stacy-efs \
  --description "Stacy EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name stacy-db-subnet \
  --db-subnet-group-description "Stacy RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier stacy-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username stacy \
...
DATABASE_URL="postgresql://stacy:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/stacy"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=stacy-data \
...
aws secretsmanager create-secret \
  --name stacy/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name stacy/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name stacy/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name stacy/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name stacy/github-token \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:stacy/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name stacy-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name stacy

aws logs create-log-group --log-group-name /ecs/stacy
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$STACY_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/stacy-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/stacy-task-def.json
```
...
- Create a CNAME or ALIAS record for `$STACY_DOMAIN` -> `$ALB_DNS`
...
aws ecs create-service \
  --cluster stacy \
  --service-name stacy-server \
  --task-definition stacy-server \
  --desired-count 1 \
  --launch-type FARGATE \
...
aws ecs describe-services \
  --cluster stacy \
  --services stacy-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster stacy --service-name stacy-server
TASK_ARN=$(aws ecs list-tasks --cluster stacy --service-name stacy-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster stacy --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/stacy --since 10m --follow

# Hit the health endpoint
curl -sf https://$STACY_DOMAIN/api/health
```
...
aws ecs update-service \
  --cluster stacy \
  --service stacy-server \
  --task-definition stacy-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier stacy-db
aws rds start-db-instance --db-instance-identifier stacy-db
```
...
aws ecs update-service --cluster stacy --service stacy-server --desired-count 0
aws ecs delete-service --cluster stacy --service stacy-server --force
aws ecs delete-cluster --cluster stacy
...
aws rds delete-db-instance \
  --db-instance-identifier stacy-db \
  --final-db-snapshot-identifier stacy-db-final
aws rds wait db-instance-deleted --db-instance-identifier stacy-db
aws rds delete-db-subnet-group --db-subnet-group-name stacy-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id stacy/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name stacy-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name stacy-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name stacy-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name stacy-ecs-execution
aws iam delete-role --role-name stacy-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/stacy
```
