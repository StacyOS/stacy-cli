...
Stacy companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .stacy.yaml     # Adapter config, env inputs, routines
```
...
- **.stacy.yaml** holds Stacy-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
stacy company export <company-id> --out ./my-export
```
...
stacy company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
stacy company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
stacy company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.stacy.yaml`
...
```sh
# From a local folder
stacy company import ./my-export

# From a GitHub URL
stacy company import https://github.com/org/repo

# From a GitHub subfolder
stacy company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
stacy company import org/repo
stacy company import org/repo/companies/acme
```
...
If `--target` is not specified, Stacy infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
stacy company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
stacy company import org/company-templates/engineering-team \
  --target new \
...
```sh
stacy company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
stacy company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
stacy company import ./package \
  --target new \
  --yes \
  --json
```
...
Stacy supports several GitHub URL formats:
