...
How to get OpenClaw running in a Docker container for local development and testing the Stacy OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Stacy includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-stacy-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Stacy host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-stacy-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `STACY_HOST_PORT` (default `3100`)
- `STACY_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Stacy deployment is `authenticated`, provide auth context:

```bash
STACY_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
STACY_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Stacy server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Stacy URL (typically `http://host.docker.internal:3100`).
- If Stacy rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm stacy allowed-hostname host.docker.internal
```

Then restart Stacy and rerun the smoke script.
