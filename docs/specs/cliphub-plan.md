# ClipHub: Marketplace for Stacy Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Stacy configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Stacy-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Stacy company (agents, projects, governance) |
...
A complete Stacy company configuration:
...
Single-agent configurations designed to plug into a Stacy org:
...
Portable skill files that any Stacy agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Stacy's skill loading system
...
interface Listing {
  ...
  stacyVersionMin: string;     // Minimum Stacy version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Stacy company
...
| `POST` | `/api/listings/:id/install` | Install to Stacy company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Stacy company (or create new)
  → ClipHub API calls Stacy API to:
  ...
  → Redirect to Stacy dashboard with new team running
```
...
### 5.3 Creator: Export from Stacy → Publish

```
Running Stacy company → "Export as Blueprint" (CLI or UI)
  → Stacy exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Stacy brand blue/purple)
...
| Install flow | One-click deploy to Stacy company |
...
{
  "targetCompanyId": "uuid",         // Existing Stacy company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `stacy-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Stacy
- **Backend**: Node.js API (or extend Stacy server)
- **Database**: Postgres (can share Stacy's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Stacy auth (or OAuth2)

### 9.2 Integration with Stacy

ClipHub can be:
- **Option A**: A separate app that calls Stacy's API to install blueprints
- **Option B**: A built-in section of the Stacy UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Stacy UI and API.
...
- [ ] Install flow: blueprint → Stacy company
