...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Stacy-Signature` + `X-Stacy-Timestamp` header pair matching the trigger's signing mode.
