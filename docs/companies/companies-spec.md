...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Stacy.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.stacy.yaml`
...
- Stacy must not require extra top-level fields for skill validity
- Stacy-specific extensions must live under `metadata.stacy` or `metadata.sources`
...
For Stacy, the preferred fidelity extension is:
...
- issue/task Stacy-only metadata
...
- Stacy tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Stacy shape:
...
Additional rules for Stacy exporters:
...
- omit empty and default-valued Stacy fields when possible
...
- Stacy exporters should emit `.stacy.yaml` as a sidecar by default
...
Stacy can map this spec to its runtime model like this:
...
- Stacy extension:
  - `.stacy.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Stacy-specific fidelity

Inline Stacy-only metadata that must live inside a shared markdown file should use:

- `metadata.stacy`

That keeps the base format broader than Stacy.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Stacy.

## 20. Cutover

Stacy should cut over to this markdown-first package model as the primary portability format.

`stacy.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Stacy, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
