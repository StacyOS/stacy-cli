...
const PORT = Number(process.env.STACY_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const STACY_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "stacy-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp STACY_HOME.
  webServer: {
    command: `pnpm stacy onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active Stacy home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      STACY_HOME,
      STACY_INSTANCE_ID: "playwright-e2e",
      STACY_BIND: "loopback",
      STACY_DEPLOYMENT_MODE: "local_trusted",
