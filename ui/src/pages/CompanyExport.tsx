...
/**
 * Filter .stacy.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterStacyYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://stacy.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm stacy company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Stacy](https://stacy.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Stacy](https://stacy.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .stacy.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .stacy.yaml
    const yamlPath = exportData.stacyExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterStacyYaml(exportData.files[yamlPath], checkedFiles);
    }
