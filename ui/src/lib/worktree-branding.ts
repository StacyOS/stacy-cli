...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("stacy-worktree-enabled") !== "true") return null;

  const name = readMetaContent("stacy-worktree-name");
  const color = normalizeHexColor(readMetaContent("stacy-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("stacy-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
