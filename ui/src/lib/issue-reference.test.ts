...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal Stacy issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/arpanstacy/stacy/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/arpanstacy/stacy/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute Stacy issue URLs so origin, port, and hash are not lost", () => {
