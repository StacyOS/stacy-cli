...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Stacy API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "stacyApiUrl",
                  String(config.stacyApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "stacyApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://stacy.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/stacy-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "stacy"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Stacy persists a device key during onboarding so pairing approvals
