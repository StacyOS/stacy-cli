...
type StacySprite = (typeof STACY_SPRITES)[number];
...
function spriteSize(sprite: StacySprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = STACY_SPRITES[(baseCol + baseRow) % STACY_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = STACY_SPRITES[Math.floor(Math.random() * STACY_SPRITES.length)]!;
