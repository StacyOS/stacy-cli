...
import { buildProjectMentionHref, buildSkillMentionHref } from "@arpanstacy/stacy-shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Stacy App", "Ping @Stacy App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Stacy App",
      atPos: 5,
      endPos: "Ping @Stacy App".length,
    });
  });
  ...
  it("accepts mention selection from touchstart taps", async () => {
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("Stacy App"));
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Stacy App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
