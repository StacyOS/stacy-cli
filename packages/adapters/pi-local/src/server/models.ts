...
import type { AdapterModel } from "@arpanstacy/stacy-adapter-utils";
import { asString, runChildProcess } from "@arpanstacy/stacy-adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.STACY_PI_COMMAND === "string" &&
    process.env.STACY_PI_COMMAND.trim().length > 0
      ? process.env.STACY_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["STACY_", "npm_", "NPM_"] as const;
