import { asBoolean, asString, asStringArray } from "@arpanstacy/stacy-adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Stacy will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
