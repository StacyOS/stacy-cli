...
## Scope
This plan is now **gateway-only**. Stacy supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Stacy skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Stacy task.
...
```bash
pnpm --filter @arpanstacy/stacy-server test:run -- openclaw-gateway
pnpm --filter @arpanstacy/stacy-server typecheck
pnpm --filter @arpanstacy/stacy-ui typecheck
pnpm --filter /stacy typecheck
```
