...
Use when:
- You want Stacy to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Stacy server.
...
- stacyApiUrl (string, optional): absolute Stacy base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/stacy-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default stacy)

Standard outbound payload additions:
- stacy (object): standardized Stacy context added to every gateway agent request
- stacy.workspace (object, optional): resolved execution workspace for this run
- stacy.workspaces (array, optional): additional workspace hints Stacy exposed to the run
- stacy.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
