...
import { asString, parseObject } from "@arpanstacy/stacy-adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Stacy server host.",
        });
      }
    } catch (err) {
      checks.push({
