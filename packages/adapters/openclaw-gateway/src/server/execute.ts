...
const DEFAULT_CLIENT_VERSION = "stacy";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "stacy";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`stacy:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`stacy:issue:${input.issueId}`, input.agentId);
  }
...
function resolveStacyApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/stacy-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildStacyEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const stacyApiUrlOverride = resolveStacyApiUrlOverride(ctx.config.stacyApiUrl);
  const stacyEnv: Record<string, string> = {
    ...buildStacyEnv(ctx.agent),
    STACY_RUN_ID: ctx.runId,
  };

  if (stacyApiUrlOverride) {
    stacyEnv.STACY_API_URL = stacyApiUrlOverride;
  }
  if (wakePayload.taskId) stacyEnv.STACY_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) stacyEnv.STACY_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) stacyEnv.STACY_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) stacyEnv.STACY_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) stacyEnv.STACY_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    stacyEnv.STACY_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  stacyEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/stacy-claimed-api-key.json";
  const orderedKeys = [
    "STACY_RUN_ID",
    "STACY_AGENT_ID",
    "STACY_COMPANY_ID",
    ...
    "STACY_TASK_ID",
    "STACY_WAKE_REASON",
    "STACY_WAKE_COMMENT_ID",
    "STACY_APPROVAL_ID",
    "STACY_APPROVAL_STATUS",
    "STACY_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = stacyEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = stacyEnv.STACY_API_URL ?? "<set STACY_API_URL>";

  const lines = [
    "Stacy wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `STACY_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load STACY_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $STACY_API_KEY on every API call.",
    "- Use X-Stacy-Run-Id: $STACY_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: STACY_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$STACY_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$STACY_COMPANY_ID/issues?assigneeAgentId=$STACY_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardStacyPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  stacyEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateStacy = parseObject(payloadTemplate.stacy);
  const workspace = asRecord(ctx.context.stacyWorkspace);
  const workspaces = Array.isArray(ctx.context.stacyWorkspaces)
    ? ctx.context.stacyWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.stacyRuntimeServiceIntents)
    ? ctx.context.stacyRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardStacy: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: stacyEnv.STACY_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.stacyWake);
  if (Object.keys(structuredWake).length > 0) {
    standardStacy.wake = structuredWake;
  }

  if (workspace) {
    standardStacy.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardStacy.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardStacy.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "stacy-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const stacyEnv = buildStacyEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderStacyWakePrompt(ctx.context.stacyWake);
  const structuredWakeJson = stringifyStacyWakePayload(ctx.context.stacyWake);
  ...
  const stacyPayload = buildStandardStacyPayload(ctx, wakePayload, stacyEnv, payloadTemplate);
  ...
  agentParams.stacy = stacyPayload;
