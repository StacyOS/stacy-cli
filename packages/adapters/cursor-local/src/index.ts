...
Use when:
- You want Stacy to run Cursor Agent CLI locally as the agent runtime
...
- Stacy auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$stacy" and related skills on local runs.
- Stacy auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
