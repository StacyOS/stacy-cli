...
import type { AdapterExecutionContext } from "@arpanstacy/stacy-adapter-utils";
import { ensureStacySkillSymlink, type StacySkillEntry } from "@arpanstacy/stacy-adapter-utils/server-utils";

const DEFAULT_STACY_INSTANCE_ID = "default";

type SkillEntry = StacySkillEntry;
...
function resolveManagedClaudePromptCacheRoot(
  ...
  const stacyHome = nonEmpty(env.STACY_HOME) ?? path.resolve(os.homedir(), ".stacy");
  const instanceId = nonEmpty(env.STACY_INSTANCE_ID) ?? DEFAULT_STACY_INSTANCE_ID;
  return path.resolve(
    stacyHome,
    "instances",
    instanceId,
...
async function buildClaudePromptBundleKey(input: {
  ...
  hash.update("stacy-claude-prompt-bundle:v1\n");
...
export async function prepareClaudePromptBundle(input: {
  ...
  for (const entry of skills) {
    ...
    try {
      await ensureStacySkillSymlink(entry.source, target);
    } catch (err) {
      await onLog(
        "stderr",
        `[stacy] Failed to materialize Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
