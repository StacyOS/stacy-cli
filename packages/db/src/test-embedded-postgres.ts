...
const DEFAULT_STACY_EMBEDDED_POSTGRES_PORT = 54329;

function getReservedTestPorts(): Set<number> {
  const configuredPorts = [
    DEFAULT_STACY_EMBEDDED_POSTGRES_PORT,
    Number.parseInt(process.env.STACY_EMBEDDED_POSTGRES_PORT ?? "", 10),
    ...String(process.env.STACY_TEST_POSTGRES_RESERVED_PORTS ?? "")
      .split(",")
...
async function getAvailablePort(): Promise<number> {
  ...
  throw new Error(
    `Failed to allocate embedded Postgres test port outside reserved Stacy ports: ${[
      ...reservedPorts,
    ].join(", ")}`,
  );
}
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const connectionString = `postgres://stacy:stacy@127.0.0.1:${port}/stacy`;
