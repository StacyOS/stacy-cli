...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://stacy:stacy@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "stacy");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://stacy:stacy@127.0.0.1:${preferredPort}/stacy`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://stacy:stacy@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "stacy");
    return {
      connectionString: `postgres://stacy:stacy@127.0.0.1:${port}/stacy`,
  ...
  const adminConnectionString = `postgres://stacy:stacy@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "stacy");

  return {
    connectionString: `postgres://stacy:stacy@127.0.0.1:${selectedPort}/stacy`,
