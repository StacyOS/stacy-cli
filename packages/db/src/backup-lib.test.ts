...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("stacy-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("stacy-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "stacy_restore_target",
      );
      const backupDir = createTempDir("stacy-db-backup-output-");
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          filenamePrefix: "stacy-test",
          backupEngine: "javascript",
        });

        expect(result.backupFile).toMatch(/stacy-test-.*\.sql\.gz$/);
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("stacy-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- Stacy database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- stacy statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
