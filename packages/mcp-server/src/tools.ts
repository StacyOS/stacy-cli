...
async function getIssueWorkspaceRuntime(client: StacyApiClient, issueId: string) {
...
export function createToolDefinitions(client: StacyApiClient): ToolDefinition[] {
  return [
    makeTool(
      "stacyMe",
      "Get the current authenticated Stacy actor details",
      z.object({}),
    ...
    makeTool(
      "stacyApiRequest",
      "Make a JSON request to an existing Stacy /api endpoint for unsupported operations",
      apiRequestSchema,
