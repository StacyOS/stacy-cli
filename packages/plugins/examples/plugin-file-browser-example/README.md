...
Example Stacy plugin that demonstrates:
...
```bash
pnpm --filter @arpanstacy/stacy-plugin-file-browser-example build
pnpm stacy plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm stacy plugin uninstall stacy-file-browser-example --force
```
...
- Optional: use `stacy-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
