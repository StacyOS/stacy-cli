...
import type { Goal, Issue } from "@arpanstacy/stacy-shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the Stacy APIs.",
...
const plugin: StacyPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
