/**
 * Core types for the Stacy plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@arpanstacy/stacy-plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @arpanstacy/stacy-shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Stacy object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Stacy UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Stacy domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Stacy domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Stacy secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Stacy secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@arpanstacy/stacy-plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@arpanstacy/stacy-plugin-sdk";
 *
