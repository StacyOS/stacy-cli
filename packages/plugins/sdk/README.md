# `@arpanstacy/stacy-plugin-sdk`

Official TypeScript SDK for Stacy plugin authors.

- **Worker SDK:** `@arpanstacy/stacy-plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@arpanstacy/stacy-plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@arpanstacy/stacy-plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@arpanstacy/stacy-plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@arpanstacy/stacy-plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@arpanstacy/stacy-plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@arpanstacy/stacy-plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@arpanstacy/stacy-plugin-sdk/ui/hooks` | Hooks only |
| `@arpanstacy/stacy-plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@arpanstacy/stacy-plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@arpanstacy/stacy-plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@arpanstacy/stacy-plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@arpanstacy/stacy-plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@arpanstacy/stacy-plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @arpanstacy/stacy-plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Stacy app. They can call ordinary Stacy HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Stacy instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@arpanstacy/stacy-plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@arpanstacy/stacy-plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Stacy information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@arpanstacy/stacy-plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:stacy.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@arpanstacy/stacy-plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@arpanstacy/stacy-plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@arpanstacy/stacy-plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@arpanstacy/stacy-plugin-sdk/ui";
import type { PluginDetailTabProps } from "@arpanstacy/stacy-plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@arpanstacy/stacy-plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@arpanstacy/stacy-plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@arpanstacy/stacy-plugin-sdk/ui";
...
import {
  useHostContext,
  usePluginAction,
} from "@arpanstacy/stacy-plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@arpanstacy/stacy-plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@arpanstacy/stacy-plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@arpanstacy/stacy-plugin-sdk/bundlers";
...
```bash
stacy-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@arpanstacy/stacy-plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__stacy__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__stacy__/events` streams `reload` SSE events on UI build changes
