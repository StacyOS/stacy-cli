...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".stacy-sdk");
...
/**
 * Generate a complete Stacy plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Stacy plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "stacy-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["stacy", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@arpanstacy/stacy-shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@arpanstacy/stacy-plugin-sdk": sdkDependency,
...
`,
  ...
  if (template === "environment") {
    writeFile(
      ...
      `import type { StacyPluginManifestV1 } from "@arpanstacy/stacy-plugin-sdk";

const manifest: StacyPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@arpanstacy/stacy-plugin-sdk/ui";
...
} from "@arpanstacy/stacy-plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@arpanstacy/stacy-plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@arpanstacy/stacy-plugin-sdk\` and \`@arpanstacy/stacy-shared\` from a local Stacy checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.stacy-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@arpanstacy/stacy-plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-stacy-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <stacy-sdk-path>]");
    process.exit(1);
  }
