...
export function parseSandboxRemoteExecutionSpec(value: unknown): SandboxRemoteExecutionSpec | null {
  ...
  return {
    ...
    stacyApiUrl: asString(parsed.stacyApiUrl).trim() || null,
  };
}
...
export function buildSandboxExecutionSessionIdentity(spec: SandboxRemoteExecutionSpec | null) {
  ...
  return {
    ...
    ...(spec.stacyApiUrl ? { stacyApiUrl: spec.stacyApiUrl } : {}),
  } as const;
}
...
export function sandboxExecutionSessionMatches(saved: unknown, current: SandboxRemoteExecutionSpec | null): boolean {
  ...
  return (
    ...
    asString(parsedSaved.stacyApiUrl) === asString(currentIdentity.stacyApiUrl)
  );
}
...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".stacy-runtime", input.adapterKey);

  await withTempDir("stacy-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".stacy-runtime", ...(input.preserveAbsentOnRestore ?? [])]);
  ...
  return {
    ...
    restoreWorkspace: async () => {
      await withTempDir("stacy-sandbox-restore-", async (tempDir) => {
        ...
        await mirrorDirectory(extractedDir, input.workspaceLocalDir, {
          preserveAbsent: [".stacy-runtime", ...(input.preserveAbsentOnRestore ?? [])],
        });
      });
    },
  };
