...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.STACY_SECRETS_PROVIDER === undefined) {
    process.env.STACY_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.STACY_SECRETS_STRICT_MODE === undefined) {
    process.env.STACY_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.STACY_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.STACY_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.STACY_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.STACY_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set STACY_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@stacy.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.STACY_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://stacy:stacy@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "stacy");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://stacy:stacy@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "stacy");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: stacy");
    }
  
    const embeddedConnectionString = `postgres://stacy:stacy@127.0.0.1:${port}/stacy`;
  ...
  process.env.STACY_LISTEN_PORT = String(listenPort);
  process.env.STACY_RUNTIME_API_URL = runtimeApiUrl;
  process.env.STACY_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.STACY_API_URL = configuredApiUrl;
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Stacy server failed to start");
    process.exit(1);
  });
}
