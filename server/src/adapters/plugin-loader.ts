...
function extractUiParserSource(
  ...
  const contractVersion = pkg.stacy?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no stacy.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
