...
import { buildStacyEnv } from "../adapters/utils.js";
...
describe("buildStacyEnv", () => {
  it("prefers an explicit STACY_RUNTIME_API_URL", () => {
    process.env.STACY_RUNTIME_API_URL = "http://100.104.161.29:3102";
    process.env.STACY_API_URL = "http://localhost:4100";
    process.env.STACY_LISTEN_HOST = "127.0.0.1";
    process.env.STACY_LISTEN_PORT = "3101";

    const env = buildStacyEnv({ id: "agent-1", companyId: "company-1" });
  ...
  it("falls back to STACY_API_URL when no runtime URL is configured", () => {
    delete process.env.STACY_RUNTIME_API_URL;
    process.env.STACY_API_URL = "http://localhost:4100";
    process.env.STACY_LISTEN_HOST = "127.0.0.1";
    process.env.STACY_LISTEN_PORT = "3101";
    ...
    expect(env.STACY_API_URL).toBe("http://localhost:4100");
  });

  it("uses runtime listen host/port when explicit URL is not set", () => {
    delete process.env.STACY_API_URL;
    process.env.STACY_LISTEN_HOST = "0.0.0.0";
    process.env.STACY_LISTEN_PORT = "3101";
    process.env.PORT = "3100";

    const env = buildStacyEnv({ id: "agent-1", companyId: "company-1" });

    expect(env.STACY_API_URL).toBe("http://localhost:3101");
  });

  it("formats IPv6 hosts safely in fallback URL generation", () => {
    delete process.env.STACY_API_URL;
    process.env.STACY_LISTEN_HOST = "::1";
    process.env.STACY_LISTEN_PORT = "3101";

    const env = buildStacyEnv({ id: "agent-1", companyId: "company-1" });

    expect(env.STACY_API_URL).toBe("http://[::1]:3101");
  });

  it("accepts STACY runtime configuration", () => {
    delete process.env.STACY_RUNTIME_API_URL;
    delete process.env.STACY_API_URL;
    delete process.env.STACY_LISTEN_HOST;
    delete process.env.STACY_LISTEN_PORT;
