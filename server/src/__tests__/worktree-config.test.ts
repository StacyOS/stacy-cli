...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const stacyDir = path.join(worktreeRoot, ".stacy");
    const configPath = path.join(stacyDir, "config.json");
    const envPath = path.join(stacyDir, ".env");
    const sharedRoot = path.join(tempRoot, ".stacy", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".stacy-worktrees");

    await fs.mkdir(stacyDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Stacy environment variables",
        "STACY_IN_WORKTREE=true",
        "STACY_WORKTREE_NAME=PAP-884-ai-commits-component",
        "STACY_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.STACY_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.STACY_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`STACY_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('STACY_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`STACY_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`STACY_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('STACY_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.STACY_HOME).toBe(isolatedHome);
    expect(process.env.STACY_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Stacy environment variables",
        "STACY_IN_WORKTREE=true",
        "STACY_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.STACY_IN_WORKTREE = "true";
    process.env.STACY_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.STACY_WORKTREES_DIR = isolatedHome;
    delete process.env.STACY_HOME;
    delete process.env.STACY_INSTANCE_ID;
    delete process.env.STACY_CONFIG;
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-worktree-runtime-override-"));
    const isolatedHome = path.join(tempRoot, ".stacy-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        "# Stacy environment variables",
        `STACY_HOME=${JSON.stringify(isolatedHome)}`,
        `STACY_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `STACY_CONFIG=${JSON.stringify(configPath)}`,
        `STACY_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'STACY_IN_WORKTREE="true"',
        'STACY_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.STACY_IN_WORKTREE = "true";
    process.env.STACY_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.STACY_HOME = transientHome;
    ...
    expect(repairedEnv).toContain(`STACY_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`STACY_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.STACY_HOME).toBe(isolatedHome);
  });

  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".stacy-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".stacy", "worktrees");
    ...
    const stacyDir = path.join(currentWorktreeRoot, ".stacy");
    const configPath = path.join(stacyDir, "config.json");
    const envPath = path.join(stacyDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".stacy", "config.json");
  ...
  it("persists runtime-selected worktree ports back into explicit-port auth URLs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.STACY_IN_WORKTREE = "true";
    process.env.STACY_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.STACY_HOME = isolatedHome;
    process.env.STACY_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.STACY_CONFIG = configPath;
  ...
  it("does not rewrite no-port public auth URLs when persisting runtime-selected ports", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-worktree-public-ports-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        {
          ...buildLegacyConfig(instanceRoot, "https://stacy.example"),
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: path.join(instanceRoot, "db"),
            embeddedPostgresPort: 54331,
    ...
    process.chdir(worktreeRoot);
    process.env.STACY_IN_WORKTREE = "true";
    process.env.STACY_WORKTREE_NAME = "PAP-125-public-base-url";
    process.env.STACY_HOME = isolatedHome;
    process.env.STACY_INSTANCE_ID = "pap-125-public-base-url";
    process.env.STACY_CONFIG = configPath;
    ...
    expect(writtenConfig.server.port).toBe(3103);
    expect(writtenConfig.database.embeddedPostgresPort).toBe(54335);
    expect(writtenConfig.auth.publicBaseUrl).toBe("https://stacy.example");
  });
  ...
  it("does not rewrite the in-memory config when auth URL has no explicit port", () => {
    const { config, changed } = applyRuntimePortSelectionToConfig(
      buildLegacyConfig("/tmp/shared", "https://stacy.example"),
      {
        serverPort: 3104,
        databasePort: 54340,
        allowServerPortWrite: false,
    ...
    expect(config.server.port).toBe(3100);
    expect(config.database.embeddedPostgresPort).toBe(54340);
    expect(config.auth.publicBaseUrl).toBe("https://stacy.example");
  });
});
