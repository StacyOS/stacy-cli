...
<head>
    <!-- STACY_RUNTIME_BRANDING_START -->
    <!-- STACY_RUNTIME_BRANDING_END -->
    <!-- STACY_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from STACY_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ STACY_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ STACY_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ STACY_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      STACY_IN_WORKTREE: "true",
      STACY_WORKTREE_NAME: "stacy-pr-432",
      STACY_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("stacy-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="stacy-worktree-name"');
    expect(meta).toContain('content="stacy-pr-432"');
    expect(meta).toContain('name="stacy-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="stacy-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="stacy-worktree-name"');
  });
});
