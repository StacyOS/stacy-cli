...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "stacy", LOGNAME: "stacy", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "stacy" }),
      },
    );

    expect(tokens).toEqual(["stacy", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "stacy" }, {
        userInfo: () => ({ username: "stacy" }),
      });

      expect(tokens).toEqual(["stacy", "custom-token"]);
    } finally {
