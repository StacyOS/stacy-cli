...
describe.sequential("cli auth routes", () => {
  ...
  it.sequential("creates a CLI auth challenge with approval metadata", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        command: "stacy company import",
        clientName: "stacy cli",
  ...
  it.sequential("rejects anonymous access to generic skill documents", async () => {
    ...
    const skillRes = await request(skillApp).get("/api/skills/stacy");
  ...
  it.sequential("serves the invite-scoped stacy skill anonymously for active invites", async () => {
    const invite = {
      id: "invite-1",
      companyId: "company-1",
    ...
    const res = await request(app).get("/api/invites/token-123/skills/stacy");

    expect(res.status).toBe(200);
    expect(res.headers["content-type"]).toContain("text/markdown");
    expect(res.text).toContain("# Stacy Skill");
  });
