...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Stacy OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/stacy");
    expect(text).toContain("Suggested Stacy base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("stacyApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.stacyApiUrl");
    expect(text).toContain("~/.openclaw/workspace/stacy-claimed-api-key.json");
    expect(text).toContain("STACY_API_KEY");
