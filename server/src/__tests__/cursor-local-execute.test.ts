...
import { execute } from "@arpanstacy/stacy-adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects stacy env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the stacy heartbeat.");
      ...
      expect(capture.prompt).toContain("Stacy runtime note:");
      expect(capture.prompt).toContain("STACY_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "stacy-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const stacyDir = await createSkillDir(runtimeSkillsRoot, "stacy");
