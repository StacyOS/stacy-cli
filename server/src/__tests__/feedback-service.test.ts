...
import { writeStacySkillSyncPreference } from "@arpanstacy/stacy-adapter-utils/server-utils";
import {
  agents,
  companies,
  companySkills,
...
describe("feedbackService.saveIssueVote", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("stacy-feedback-service-");
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "stacy-feedback-instructions-"));
    ...
    await db.insert(agents).values({
      ...
      status: "active",
      adapterType: "codex_local",
      adapterConfig: writeStacySkillSyncPreference(
        {
          model: "gpt-5.4",
        ...
        ["stacy/skills/stacy", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
  ...
  it("builds a detailed sanitized shared bundle with issue and agent context", async () => {
    ...
    expect(trace?.schemaVersion).toBe("stacy-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("stacy-feedback-bundle-v2");
    ...
    expect(skillItems?.some((item) => item.sourceLocator === "https://github.com/octo/research/tree/main/skills/public-skill")).toBe(true);
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("stacy-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("stacy-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "stacy-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "stacy-feedback-opencode-"));
    ...
    vi.stubEnv("STACY_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,
  ...
  it("flushes pending shared traces into configured object storage and marks them sent", async () => {
    ...
    expect(uploadTraceBundle.mock.calls[0]?.[0]).toMatchObject({
      ...
      envelope: {
        destination: "stacy_labs_feedback_v1",
        exportId: traces[0]?.exportId,
      },
    });
  });
