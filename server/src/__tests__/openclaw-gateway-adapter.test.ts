...
import { execute, testEnvironment } from "@arpanstacy/stacy-adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@arpanstacy/stacy-adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("stacy:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("STACY_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("STACY_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Stacy Wake Payload");
      ...
      expect(payload?.stacy).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
