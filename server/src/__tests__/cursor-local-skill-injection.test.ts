...
import { ensureCursorSkillsInjected } from "@arpanstacy/stacy-adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Stacy skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("stacy-cursor-skills-src-");
    const skillsHome = await makeTempDir("stacy-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "stacy");
    await createSkillDir(skillsDir, "stacy-create-agent");
    ...
    const injectedA = path.join(skillsHome, "stacy");
    const injectedB = path.join(skillsHome, "stacy-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "stacy")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "stacy-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "stacy"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "stacy-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("stacy-cursor-preserve-src-");
    const skillsHome = await makeTempDir("stacy-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "stacy");
    ...
    expect((await fs.lstat(path.join(skillsHome, "stacy-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("stacy-cursor-fail-src-");
    const skillsHome = await makeTempDir("stacy-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
