...
describe("agent instructions service", () => {
  const originalStacyHome = process.env.STACY_HOME;
  const originalStacyInstanceId = process.env.STACY_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalStacyHome === undefined) delete process.env.STACY_HOME;
    else process.env.STACY_HOME = originalStacyHome;
    if (originalStacyInstanceId === undefined) delete process.env.STACY_INSTANCE_ID;
    else process.env.STACY_INSTANCE_ID = originalStacyInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const stacyHome = await makeTempDir("stacy-agent-instructions-home-");
    const externalRoot = await makeTempDir("stacy-agent-instructions-external-");
    cleanupDirs.add(stacyHome);
    cleanupDirs.add(externalRoot);
    process.env.STACY_HOME = stacyHome;
    process.env.STACY_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("stacy-agent-instructions-new-external-");
    cleanupDirs.add(stacyHome);
    cleanupDirs.add(externalRoot);
    process.env.STACY_HOME = stacyHome;
    process.env.STACY_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("stacy-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const stacyHome = await makeTempDir("stacy-agent-instructions-recover-");
    cleanupDirs.add(stacyHome);
    process.env.STACY_HOME = stacyHome;
    process.env.STACY_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const stacyHome = await makeTempDir("stacy-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("stacy-agent-instructions-stale-root-");
    cleanupDirs.add(stacyHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const stacyHome = await makeTempDir("stacy-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("stacy-agent-instructions-heal-write-stale-");
    cleanupDirs.add(stacyHome);
    cleanupDirs.add(staleRoot);
    process.env.STACY_HOME = stacyHome;
    process.env.STACY_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const stacyHome = await makeTempDir("stacy-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("stacy-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(stacyHome);
    cleanupDirs.add(staleRoot);
    process.env.STACY_HOME = stacyHome;
    process.env.STACY_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const stacyHome = await makeTempDir("stacy-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("stacy-agent-instructions-partial-root-");
    cleanupDirs.add(stacyHome);
    cleanupDirs.add(staleRoot);
    process.env.STACY_HOME = stacyHome;
    process.env.STACY_INSTANCE_ID = "test-instance";
