...
describe("agent local JWT", () => {
  const secretEnv = "STACY_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "STACY_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "STACY_AGENT_JWT_ISSUER";
  const audienceEnv = "STACY_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when STACY_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "stacy";
    process.env[audienceEnv] = "stacy-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
