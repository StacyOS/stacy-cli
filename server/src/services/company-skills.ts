...
import { companies, companySkills } from "@arpanstacy/stacy-db";
import { readStacySkillSyncPreference } from "@arpanstacy/stacy-adapter-utils/server-utils";
import type { StacySkillEntry } from "@arpanstacy/stacy-adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@arpanstacy/stacy-shared";
import { resolveStacyInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("stacy/skills/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveStacyInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "stacy_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Stacy skills are read-only.",
      sourceLabel: "Stacy bundled",
      sourceBadge: "stacy",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "stacy_bundled";
      out.push({
        key: skill.key,
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "stacy_bundled"
        && incomingKind === "github"
        && incomingOwner === "stacy"
        && incomingRepo === "stacy"
      ) {
