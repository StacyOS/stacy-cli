...
export function pluginDatabaseService(db: Db) {
  ...
  async function ensureNamespace(pluginId: string, manifest: StacyPluginManifestV1) {
  ...
  return {
    ...
    async applyMigrations(pluginId: string, manifest: StacyPluginManifestV1, packageRoot: string) {
      if (!manifest.database) return null;
