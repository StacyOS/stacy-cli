...
import { ensureOpenCodeModelConfiguredAndAvailable } from "@arpanstacy/stacy-adapter-opencode-local/server";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".stacy.yaml" || normalized === ".stacy.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const stacy = isPlainRecord(metadata?.stacy) ? metadata?.stacy as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.stacySkillKey)
    ?? asString(stacy?.skillKey)
    ?? asString(stacy?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "stacy_bundled") {
    return `stacy/skills/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .stacy.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .stacy.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .stacy.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .stacy.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Stacy will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .stacy.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .stacy.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .stacy.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .stacy.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .stacy.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .stacy.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .stacy.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .stacy.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .stacy.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[stacyExtensionPath];
  if (selectedFiles.has(stacyExtensionPath) && typeof extensionEntry === "string") {
    filtered[stacyExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findStacyExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".stacy.yaml"] === "string") return ".stacy.yaml";
  if (typeof files[".stacy.yml"] === "string") return ".stacy.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.stacy.yaml") || entry.endsWith("/.stacy.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "stacy_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "StacyOS/stacy-cli",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/StacyOS/stacy-cli/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "stacy_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.stacySkillKey = skill.key;
  metadata.stacy = {
    ...(isPlainRecord(metadata.stacy) ? metadata.stacy : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const stacyExtensionPath = findStacyExtensionPath(normalizedFiles);
  const stacyExtension = stacyExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, stacyExtensionPath) ?? "")
    : {};
  const stacyCompany = isPlainRecord(stacyExtension.company) ? stacyExtension.company : {};
  const stacySidebar = normalizePortableSidebarOrder(stacyExtension.sidebar);
  const stacyAgents = isPlainRecord(stacyExtension.agents) ? stacyExtension.agents : {};
  const stacyProjects = isPlainRecord(stacyExtension.projects) ? stacyExtension.projects : {};
  const stacyTasks = isPlainRecord(stacyExtension.tasks) ? stacyExtension.tasks : {};
  const stacyRoutines = isPlainRecord(stacyExtension.routines) ? stacyExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(stacyCompany.brandColor),
      logoPath: asString(stacyCompany.logoPath) ?? asString(stacyCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof stacyCompany.requireBoardApprovalForNewAgents === "boolean"
          ? stacyCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof stacyCompany.feedbackDataSharingEnabled === "boolean"
          ? stacyCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof stacyCompany.feedbackDataSharingConsentAt === "string"
          ? stacyCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(stacyCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(stacyCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: stacySidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(stacyAgents[slug]) ? stacyAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(stacyProjects[slug]) ? stacyProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(stacyTasks[slug]) ? stacyTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(stacyRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(stacyRoutines[slug]) ? stacyRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  const strictSecretsMode = process.env.STACY_SECRETS_STRICT_MODE === "true";
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeStacySkillSyncPreference({ ...adapterConfig }, desiredSkills);
  ...
  async function exportBundle(
    ...
    const stacyAgentsOut: Record<string, Record<string, unknown>> = {};
    const stacyProjectsOut: Record<string, Record<string, unknown>> = {};
    const stacyTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const stacyRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readStacySkillSyncPreference(
        ...
        stacyAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      stacyProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      stacyTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      stacyRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const stacyExtensionPath = ".stacy.yaml";
    const stacyAgents = Object.fromEntries(
      Object.entries(stacyAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const stacyProjects = Object.fromEntries(
      Object.entries(stacyProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const stacyTasks = Object.fromEntries(
      Object.entries(stacyTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const stacyRoutines = Object.fromEntries(
      Object.entries(stacyRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[stacyExtensionPath] = buildYamlFile(
      {
        schema: "stacy/v1",
        ...
        agents: Object.keys(stacyAgents).length > 0 ? stacyAgents : undefined,
        projects: Object.keys(stacyProjects).length > 0 ? stacyProjects : undefined,
        tasks: Object.keys(stacyTasks).length > 0 ? stacyTasks : undefined,
        routines: Object.keys(stacyRoutines).length > 0 ? stacyRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, stacyExtensionPath);
