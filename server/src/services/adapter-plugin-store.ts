 ...
 * ~/.stacy/adapter-plugins.json. This is the source of truth for which
...
import { resolveStacyHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-stacy-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const stacyDir = resolveStacyHomeDir();
  return {
    adapterPluginsDir: path.join(stacyDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(stacyDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(stacyDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "stacy-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Stacy external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
