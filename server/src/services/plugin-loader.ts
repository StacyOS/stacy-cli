 ...
 *    (`~/.stacy/plugins/`) and `node_modules` for packages matching
 *    the `stacy-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Stacy plugins.
 * Packages matching this pattern are considered Stacy plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "stacy-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: StacyPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.stacy/plugins/ local directory
  | "npm"               // npm packages matching stacy-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: StacyPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the stacy-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "stacy-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: StacyPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Stacy plugins installed as npm packages in the current
   * Node.js environment matching the "stacy-plugin-*" naming convention.
   *
   ...
   * the "stacyPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Stacy plugin.
   * Throws if the package is a Stacy plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<StacyPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: StacyPluginManifestV1;
    newManifest: StacyPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Stacy plugin naming convention.
 * Accepts both the "stacy-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @arpanstacy/stacy-plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "stacyPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const stacyPlugin = pkgJson["stacyPlugin"];
  if (
    stacyPlugin !== null &&
    typeof stacyPlugin === "object" &&
    !Array.isArray(stacyPlugin)
  ) {
    const manifestRelPath = (stacyPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: StacyPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumStacyVersion;
}
 ...
 *   packageName: "stacy-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: StacyPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as StacyPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Stacy plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a Stacy plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasStacyPlugin = "stacyPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasStacyPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<StacyPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: StacyPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @arpanstacy/stacy-shared exports). Run those workers through
