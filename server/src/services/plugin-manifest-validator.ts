 ...
 */
import { pluginManifestV1Schema } from "@arpanstacy/stacy-shared";
import type { StacyPluginManifestV1 } from "@arpanstacy/stacy-shared";
import { PLUGIN_API_VERSION } from "@arpanstacy/stacy-shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): StacyPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as StacyPluginManifestV1,
        };
      }
