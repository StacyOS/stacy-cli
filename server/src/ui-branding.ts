const FAVICON_BLOCK_START = "<!-- STACY_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- STACY_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- STACY_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- STACY_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.STACY_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.STACY_WORKTREE_NAME) ?? nonEmpty(env.STACY_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.STACY_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="stacy-worktree-enabled" content="true" />',
    `<meta name="stacy-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="stacy-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="stacy-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
