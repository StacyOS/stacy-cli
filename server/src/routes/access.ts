...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@arpanstacy/stacy-shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "stacy" &&
    normalized !== "stacy-create-agent" &&
    normalized !== "stacy-create-plugin" &&
...
/** Resolve the Stacy repo skills directory (built-in / managed skills). */
function resolveStacySkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const stacySkillsDir = resolveStacySkillsDir();

  // Build set of Stacy-managed skill names
  const stacySkillNames = new Set<string>();
  if (stacySkillsDir) {
    try {
      for (const entry of fs.readdirSync(stacySkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) stacySkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isStacyManaged: stacySkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.stacyApiUrl)) {
    const legacyStacyApiUrl = nonEmptyTrimmedString(input.stacyApiUrl);
    if (legacyStacyApiUrl) merged.stacyApiUrl = legacyStacyApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    stacyApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.stacyApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawStacyApiUrl =
    typeof defaults.stacyApiUrl === "string"
      ? defaults.stacyApiUrl.trim()
      : "";
  if (rawStacyApiUrl) {
    try {
      const parsedStacyApiUrl = new URL(rawStacyApiUrl);
      if (
        parsedStacyApiUrl.protocol !== "http:" &&
        parsedStacyApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_stacy_api_url_protocol",
          level: "warn",
          message: `stacyApiUrl must use http:// or https:// (got ${parsedStacyApiUrl.protocol}).`
        });
      } else {
        normalized.stacyApiUrl = parsedStacyApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_stacy_api_url_configured",
          level: "info",
          message: `stacyApiUrl set to ${parsedStacyApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_stacy_api_url_invalid",
        level: "warn",
        message: `Invalid stacyApiUrl: ${rawStacyApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Stacy host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Stacy is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm stacy allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/stacy`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/stacy-claimed-api-key.json and load STACY_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: stacyApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Stacy hostname is reachable and allowed via `pnpm stacy allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Stacy API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/stacy/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Stacy OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          stacyApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Stacy generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "stacyApiUrl": "https://stacy-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Stacy before key claim is allowed.
    ...
    - ~/.openclaw/workspace/stacy-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/stacy-claimed-api-key.json

    And set the STACY_API_KEY and STACY_API_URL in your environment variables as specified here:
    ...
    Then set STACY_API_KEY and STACY_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Stacy skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your STACY_API_URL to the top of your skill and note the path to your STACY_API_URL
    ...
    ${
      ...
      "Ensure Stacy is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Stacy base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.stacyApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm stacy allowed-hostname <host>
      - then restart Stacy and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@stacy.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "stacy", path: "/api/skills/stacy" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "stacy-create-agent",
          path: "/api/skills/stacy-create-agent"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "stacy",
          path: `/api/invites/${token}/skills/stacy`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "stacy") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              stacyApiUrl: req.body.stacyApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.stacyApiUrl &&
          !persistedDefaults.stacyApiUrl
        ) {
          missingPersistedFields.push("stacyApiUrl");
        }
