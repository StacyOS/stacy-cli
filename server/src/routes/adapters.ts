 ...
 * toggle server-side adapter code for the whole Stacy instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-stacy-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/stacy-hermes-adapter@0.3.0" → packageName + version
  ...
  // without modifying Stacy's source code.
  //
