...
import { heartbeatRuns, instanceUserRoles, invites } from "@arpanstacy/stacy-db";
...
function hasDevServerStatusToken(providedToken: string | undefined) {
  const expectedToken = process.env.STACY_DEV_SERVER_STATUS_TOKEN?.trim();
...
export function healthRoutes(
  ...
  router.get("/", async (req, res) => {
    ...
    const exposeDevServerDetails =
      exposeFullDetails || hasDevServerStatusToken(req.get("x-stacy-dev-server-status-token"));
