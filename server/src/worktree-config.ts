...
import type { StacyConfig } from "@arpanstacy/stacy-shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Stacy worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.STACY_IN_WORKTREE !== "true") return null;

  const configPath = resolveStacyConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.STACY_WORKTREE_NAME) ??
    nonEmpty(env.STACY_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.STACY_INSTANCE_ID) ??
    nonEmpty(env.STACY_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.STACY_HOME) ??
      nonEmpty(env.STACY_HOME) ??
      nonEmpty(env.STACY_WORKTREES_DIR) ??
      "~/.stacy-worktrees",
  );
...
function writeConfigFile(configPath: string, config: StacyConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".stacy", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as StacyConfig;
...
): { config: StacyConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.STACY_HOME = context.homeDir;
  process.env.STACY_INSTANCE_ID = context.instanceId;
  process.env.STACY_CONFIG = context.configPath;
  process.env.STACY_CONTEXT = context.contextPath;
  process.env.STACY_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as StacyConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    STACY_HOME: context.homeDir,
    STACY_INSTANCE_ID: context.instanceId,
    STACY_CONFIG: context.configPath,
    STACY_CONTEXT: context.contextPath,
    STACY_IN_WORKTREE: "true",
    STACY_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: StacyConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as StacyConfig;
  } catch {
    return;
  }
