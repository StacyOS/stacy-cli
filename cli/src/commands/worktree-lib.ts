...
export const DEFAULT_WORKTREE_HOME = "~/.stacy-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".stacy");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "stacy",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    STACY_HOME: paths.homeDir,
    STACY_INSTANCE_ID: paths.instanceId,
    STACY_CONFIG: paths.configPath,
    STACY_CONTEXT: paths.contextPath,
    STACY_IN_WORKTREE: "true",
    ...(branding?.name ? { STACY_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { STACY_WORKTREE_COLOR: branding.color } : {}),
  };
}
