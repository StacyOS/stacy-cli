...
import type { Agent } from "@arpanstacy/stacy-shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveStacySkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export STACY_API_URL='${escaped(input.apiBase)}'`,
    `export STACY_COMPANY_ID='${escaped(input.companyId)}'`,
    `export STACY_AGENT_ID='${escaped(input.agentId)}'`,
    `export STACY_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Stacy skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Stacy skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveStacySkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Stacy skills directory. Expected ./skills in the repo checkout.",
              );
            }
