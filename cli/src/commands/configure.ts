...
function defaultConfig(): StacyConfig {
...
export async function configure(opts: {
  ...
  printStacyCliBanner();
  p.intro(pc.bgCyan(pc.black(" stacy configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `pnpm stacy onboard` first.");
    p.outro("");
    return;
  }

  let config: StacyConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because STACY_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
