...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "stacy-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.STACY_HOME = path.join(root, "home");
    process.env.STACY_INSTANCE_ID = "telemetry-test";
