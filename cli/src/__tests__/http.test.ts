...
import { ApiConnectionError, ApiRequestError, StacyApiClient } from "../client/http.js";

describe("StacyApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new StacyApiClient({
    ...
    expect(headers["x-stacy-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new StacyApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("throws ApiConnectionError with recovery guidance when fetch fails", async () => {
    ...
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /Could not reach the Stacy API\./,
    );
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /curl http:\/\/localhost:3100\/api\/health/,
    );
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /pnpm dev|pnpm stacy run/,
    );
  });
