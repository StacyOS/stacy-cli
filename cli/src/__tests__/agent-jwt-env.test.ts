...
function tempConfigPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "stacy-jwt-env-"));
...
describe("agent jwt env helpers", () => {
  ...
  it("loads secret from .env next to explicit config path", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "STACY_AGENT_JWT_SECRET=test-secret\n", { mode: 0o600 });

    const loaded = readAgentJwtSecretFromEnv(configPath);
    expect(loaded).toBe("test-secret");
    expect(process.env.STACY_AGENT_JWT_SECRET).toBe("test-secret");
  });

  it("doctor check passes when secret exists in adjacent .env", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "STACY_AGENT_JWT_SECRET=check-secret\n", { mode: 0o600 });
  ...
  it("quotes hash-prefixed env values so dotenv round-trips them", () => {
    ...
    mergeStacyEnvEntries(
      {
        STACY_WORKTREE_COLOR: "#439edb",
      },
      envPath,
    );

    const contents = fs.readFileSync(envPath, "utf-8");
    expect(contents).toContain('STACY_WORKTREE_COLOR="#439edb"');
    expect(readStacyEnvEntries(envPath).STACY_WORKTREE_COLOR).toBe("#439edb");
  });
});
