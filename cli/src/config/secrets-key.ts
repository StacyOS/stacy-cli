...
import type { StacyConfig } from "./schema.js";
...
export function ensureLocalSecretsKeyFile(
  config: Pick<StacyConfig, "secrets">,
  configPath?: string,
  ...
  const envMasterKey = process.env.STACY_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    return { status: "skipped_env", path: null };
  }

  const keyFileOverride = process.env.STACY_SECRETS_MASTER_KEY_FILE;
