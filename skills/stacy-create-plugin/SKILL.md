---
name: stacy-create-plugin
description: >
  Create new Stacy plugins with the current alpha SDK/runtime. Use when
...
# Create a Stacy Plugin

Use this skill when the task is to create, scaffold, or document a Stacy plugin.
...
node packages/plugins/create-stacy-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Stacy repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.stacy-sdk/`:

```bash
pnpm --filter @arpanstacy/create-stacy-plugin build
node packages/plugins/create-stacy-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Stacy from an absolute local path during development
