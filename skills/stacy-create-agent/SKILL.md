...
description: >
  Create new agents in Stacy with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# Stacy Create Agent Skill
...
```sh
curl -sS "$STACY_API_URL/api/agents/me" \
  -H "Authorization: Bearer $STACY_API_KEY"
```

### 2. Discover adapter configuration for this Stacy instance

```sh
curl -sS "$STACY_API_URL/llms/agent-configuration.txt" \
  -H "Authorization: Bearer $STACY_API_KEY"

# Then the specific adapter you plan to use, e.g. claude_local:
curl -sS "$STACY_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $STACY_API_KEY"
```
...
```sh
curl -sS "$STACY_API_URL/api/companies/$STACY_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $STACY_API_KEY"
```
...
`skills/stacy-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/stacy-create-agent/references/baseline-role-guide.md`
...
```sh
curl -sS "$STACY_API_URL/llms/agent-icons.txt" \
  -H "Authorization: Bearer $STACY_API_KEY"
```
...
- for coding or execution agents, include the Stacy execution contract: start actionable work in the same heartbeat; do not stop at a plan unless planning was requested; leave durable progress with a clear next action; use child issues for long or parallel delegated work instead of polling; mark blocked work with owner/action; respect budget, pause/cancel, approval gates, and company boundaries
...
`skills/stacy-create-agent/references/draft-review-checklist.md`

### 8. Submit hire request

```sh
curl -sS -X POST "$STACY_API_URL/api/companies/$STACY_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $STACY_API_KEY" \
...
- when the board approves, you will be woken with `STACY_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$STACY_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $STACY_API_KEY"

curl -sS -X POST "$STACY_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $STACY_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$STACY_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $STACY_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$STACY_API_URL/api/approvals/$STACY_APPROVAL_ID" \
  -H "Authorization: Bearer $STACY_API_KEY"

curl -sS "$STACY_API_URL/api/approvals/$STACY_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $STACY_API_KEY"
```
...
- Template index and how to apply a template: `skills/stacy-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/stacy-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/stacy-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/stacy-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/stacy-create-agent/references/api-reference.md`
