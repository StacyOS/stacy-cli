...
required: false
description: >
  Develop and operate a local Stacy instance — start and stop servers,
  ...
  Stacy codebase itself or keep a running instance healthy.
---
...
> **MANDATORY:** Before running any CLI command, building, testing, or managing worktrees, you MUST read `doc/DEVELOPING.md` in the Stacy repo. It is the canonical reference for all `stacy` CLI commands, their options, build/test workflows, database operations, worktree management, and diagnostics. Do NOT guess at flags or options — read the doc first.
...
| Task | Command |
|------|---------|
| Start server (first time or normal) | `npx stacy run` |
...
| Back up database | `npx stacy db:backup` |
| Health check | `npx stacy doctor --repair` |
| Print env vars | `npx stacy env` |
| Trigger agent heartbeat | `npx stacy heartbeat run --agent-id <id>` |
| Install agent skills locally | `npx stacy agent local-cli <agent> --company-id <id>` |
...
Stacy worktrees combine git worktrees with isolated Stacy instances — each gets its own database, server port, and environment seeded from the primary instance.
...
- Starting a feature branch that needs its own Stacy environment
- Running parallel agent work without cross-contaminating the primary instance
- Testing Stacy changes in isolation before merging
...
| `worktree:list` | List worktrees and their Stacy status |
...
npx stacy worktree:make my-feature --start-point origin/main

# 2. Move into the worktree (path printed by worktree:make) and source the environment
cd <worktree-path>
eval "$(npx stacy worktree env)"

# 3. Start the isolated Stacy server
npx stacy run

# 4. Do your work

# 5. When done, merge history back if needed
npx stacy worktree:merge-history --from stacy-my-feature --to current --apply

# 6. Clean up
npx stacy worktree:cleanup my-feature
```
...
Before pushing or creating a PR, list remotes and check for one that points at a non-`stacy` GitHub fork:
...
1. **CLI is the only interface to worktrees and databases.** All worktree and database operations MUST go through `npx stacy` / `pnpm stacy` commands. You MUST NOT:
   ...
   - Run `rm -rf` on any `.stacy/`, `.stacy-worktrees/`, or `db/` directory
   - Directly manipulate embedded postgres data directories
   - Kill postgres processes by PID

2. **If a CLI command fails, stop and report.** Do NOT attempt workarounds. If `worktree:make`, `worktree reseed`, `worktree init`, `worktree:cleanup`, or any other `stacy` command fails:
   - Report the exact error message in your task comment
   - Set the task to `blocked`
   - Suggest running `npx stacy doctor --repair` or recreating the worktree from scratch
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   eval "$(npx stacy worktree env)"
   pnpm install && pnpm build
   npx stacy run          # or pnpm dev

   # If the worktree needs a fresh database:
   npx stacy worktree reseed --seed-mode full

   # If the worktree is broken beyond repair:
   npx stacy worktree:cleanup <name>
   npx stacy worktree:make <name> --seed-mode full
   ```
...
cd <worktree-path>
eval "$(npx stacy worktree env)"   # skip if using the primary instance
...
| Mistake | Fix |
|---------|-----|
| Server won't start | Run `npx stacy doctor --repair` to diagnose and auto-fix |
| Forgetting to source worktree env | Run `eval "$(npx stacy worktree env)"` after cd-ing into the worktree |
...
| Running agents against wrong instance | Verify `STACY_API_URL` points to the correct port |
