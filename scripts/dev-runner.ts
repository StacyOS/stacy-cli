...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[stacy] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`stacy worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".stacy", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-stacy-dev-server-status-token";
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[stacy] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[stacy] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[stacy] --bind custom requires --bind-host <host>");
  process.exit(1);
}
...
if (mode === "dev") {
  ...
  env.STACY_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.STACY_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.STACY_DEV_SERVER_STATUS_TOKEN;
  env.STACY_MIGRATION_PROMPT ??= "never";
  env.STACY_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[stacy] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.STACY_BIND = effectiveBind;
  if (bindHost) {
    env.STACY_BIND_HOST = bindHost;
  } else {
    delete env.STACY_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.STACY_DEPLOYMENT_MODE;
    delete env.STACY_DEPLOYMENT_EXPOSURE;
    delete env.STACY_AUTH_BASE_URL_MODE;
    console.log("[stacy] dev mode: local_trusted (bind=loopback)");
  } else {
    env.STACY_DEPLOYMENT_MODE = "authenticated";
    env.STACY_DEPLOYMENT_EXPOSURE = "private";
    env.STACY_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[stacy] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.STACY_BIND;
  delete env.STACY_BIND_HOST;
  delete env.STACY_DEPLOYMENT_MODE;
  delete env.STACY_DEPLOYMENT_EXPOSURE;
  delete env.STACY_AUTH_BASE_URL_MODE;
  console.log("[stacy] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[stacy] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@arpanstacy/stacy-db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[stacy] Command failed with code ${status.code}: pnpm --filter @arpanstacy/stacy-db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[stacy] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.STACY_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[stacy] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[stacy] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@arpanstacy/stacy-plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[stacy] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@arpanstacy/stacy-server", serverScript, ...forwardedArgs],
