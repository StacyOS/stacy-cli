...
const DEFAULT_QUERY = "\"Co-Authored-By: Stacy <noreply@stacy.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/stacy-commit-metrics-cache.json");
...
const STACY_EMAIL = "noreply@stacy.ing";
const STACY_NAME = "stacy";
...
function printHelp() {
  console.log(`Usage: tsx scripts/stacy-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === STACY_EMAIL) || (displayName && displayName.toLowerCase() === STACY_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Stacy commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "stacy-commit-metrics",
