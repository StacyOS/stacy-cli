...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Stacy data dir to use
  STACY_HOST=127.0.0.1    Host passed to the onboarded server
  STACY_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/stacy-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/stacy-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export STACY_HOST="${STACY_HOST:-127.0.0.1}"
export STACY_PORT="${STACY_PORT:-3100}"
export STACY_OPEN_ON_LISTEN="${STACY_OPEN_ON_LISTEN:-false}"
...
echo "STACY_HOST: $STACY_HOST"
echo "STACY_PORT: $STACY_PORT"
...
env \
  ...
  HOST="$STACY_HOST" \
  PORT="$STACY_PORT" \
  STACY_OPEN_ON_LISTEN="$STACY_OPEN_ON_LISTEN" \
  pnpm stacy onboard --yes --data-dir "$PC_DATA"
