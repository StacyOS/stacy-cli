...
IMAGE_NAME="${IMAGE_NAME:-stacy-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
STACY_CLI_VERSION="${STACY_CLI_VERSION:-latest}"
...
STACY_DEPLOYMENT_MODE="${STACY_DEPLOYMENT_MODE:-authenticated}"
STACY_DEPLOYMENT_EXPOSURE="${STACY_DEPLOYMENT_EXPOSURE:-private}"
...
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@stacy.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-stacy-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_STACY_CLI_VERSION=%q\n' "$STACY_CLI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e STACY_DEPLOYMENT_MODE="$STACY_DEPLOYMENT_MODE" \
      -e STACY_DEPLOYMENT_EXPOSURE="$STACY_DEPLOYMENT_EXPOSURE" \
      -e STACY_PUBLIC_URL="$STACY_PUBLIC_URL" \
      -e STACY_HOME="/stacy" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "stacy-cli" auth bootstrap-ceo --data-dir "$STACY_HOME" --base-url "$STACY_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$STACY_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$STACY_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$STACY_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$STACY_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$STACY_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg STACY_CLI_VERSION="$STACY_CLI_VERSION" \
...
echo "    Public URL: $STACY_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $STACY_DEPLOYMENT_MODE/$STACY_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/stacy-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$STACY_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $STACY_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$STACY_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $STACY_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
