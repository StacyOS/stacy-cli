#!/usr/bin/env node
/**
 * Screenshot utility for Stacy UI.
 *
 * Reads the board token from ~/.stacy/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/stacy-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".stacy/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.stacy/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Stacy origin only
