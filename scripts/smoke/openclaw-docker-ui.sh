...
STACY_HOST_PORT="${STACY_HOST_PORT:-3100}"
STACY_HOST_FROM_CONTAINER="${STACY_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.stacy-openclaw.override.yml"
...
detect_stacy_base_url() {
  ...
  for candidate in "$STACY_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${STACY_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${STACY_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
stacy_base_url="$(detect_stacy_base_url || true)"
...
Stacy URL for OpenClaw container:
EOF
  if [[ -n "$stacy_base_url" ]]; then
    cat <<EOF
  $stacy_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${STACY_HOST_PORT}
  (Do not use http://127.0.0.1:${STACY_HOST_PORT} inside the container.)
  If Stacy rejects the host, run on host machine:
    pnpm stacy allowed-hostname host.docker.internal
  Then restart Stacy and re-run this script.
EOF
  fi
  cat <<EOF
