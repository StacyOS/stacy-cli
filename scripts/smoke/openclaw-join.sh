...
COMPANY_ID="${COMPANY_ID:-${STACY_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-stacy-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-stacy-openclaw-smoke}"
...
Provide one of:
  STACY_AUTH_HEADER=\"Bearer <board-token>\"
  STACY_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Stacy OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.stacy.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
