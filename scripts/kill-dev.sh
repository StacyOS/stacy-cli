#!/usr/bin/env bash
#
# Kill all local Stacy dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all stacy dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/stacy(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.stacy/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.stacy/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.stacy/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/stacy*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.stacy/instances/*/db/postmaster.pid
    "$sibling_root"/.stacy/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 && ${#browser_pids[@]} -eq 0 ]]; then
  echo "No Stacy dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Stacy dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Stacy node processes..."
