...
base_cwd="${STACY_WORKSPACE_BASE_CWD:?STACY_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${STACY_WORKSPACE_CWD:?STACY_WORKSPACE_CWD is required}"
stacy_home="${STACY_HOME:-$HOME/.stacy}"
stacy_instance_id="${STACY_INSTANCE_ID:-default}"
stacy_dir="$worktree_cwd/.stacy"
worktree_config_path="$stacy_dir/config.json"
worktree_env_path="$stacy_dir/.env"
worktree_name="${STACY_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${STACY_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.stacy/config.json" || -L "$base_cwd/.stacy/config.json" ) ]]; then
  source_config_path="$base_cwd/.stacy/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$stacy_home/instances/$stacy_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm stacy --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm stacy worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v stacy >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      stacy worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  STACY_WORKTREES_DIR="${STACY_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const stacyDir = process.env.STACY_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.STACY_WORKTREES_DIR) ?? "~/.stacy-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(stacyDir, "config.json");
  const envPath = path.resolve(stacyDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "stacy",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.STACY_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.STACY_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.STACY_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "STACY_HOME=" + JSON.stringify(worktreeHome),
    "STACY_INSTANCE_ID=" + JSON.stringify(instanceId),
    "STACY_CONFIG=" + JSON.stringify(configPath),
    "STACY_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "STACY_IN_WORKTREE=true",
    "STACY_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.STACY_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("STACY_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  echo "Reusing existing isolated Stacy worktree config at $worktree_config_path" >&2
else
  if stacy_command_available; then
    run_isolated_worktree_init
  else
    echo "stacy CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".stacy-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
