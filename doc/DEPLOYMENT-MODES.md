...
Stacy supports two runtime modes:
...
Stacy now treats **bind** as a separate concern from auth:
...
pnpm stacy onboard --yes --bind lan
pnpm stacy run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Stacy emits a startup warning with a one-time high-entropy claim URL.
