...
Low-level reference for how Stacy packages are prepared and published to npm.
...
Stacy no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `stacy`, imports code from workspace packages such as:
...
### Manual first publish for `@arpanstacy/stacy-ui`
...
npm view @arpanstacy/stacy-ui version
...
- If `npm view @arpanstacy/stacy-ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@arpanstacy/stacy-ui`, the `stacy` npm organization must exist and the publisher must be a member with permission to publish to that scope.
