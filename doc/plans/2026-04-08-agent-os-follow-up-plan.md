...
Turn the `agent-os` research into a low-risk Stacy execution plan that preserves Stacy's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Stacy should not absorb `agent-os` as a product model or orchestration layer.

Stacy should evaluate `agent-os` in three narrow areas:
...
This work is successful when Stacy has:
...
- a proposed runtime capability model that fits current Stacy adapters
...
- replace Stacy heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Stacy core

## Existing Stacy integration points
...
- prove that Stacy can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Stacy's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Stacy-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Stacy workloads
...
- identify where Stacy should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Stacy
...
- runtime experimentation could blur the boundary between Stacy as control plane and Stacy as execution platform
