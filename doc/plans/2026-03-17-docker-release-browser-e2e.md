...
Today release smoke testing for published Stacy packages is manual and shell-driven:
...
- runs `stacy@${STACY_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@stacy.local`
- password: `stacy-smoke-password`
...
1. A published `stacy@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `stacy@latest` Docker install can be smoke-tested by Playwright in CI.
