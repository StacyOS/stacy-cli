...
This document defines the rollout plan for adapter-wide skill support in Stacy.
...
- Stacy stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Stacy currently has these adapters:
...
1. Desired skills live in Stacy for every adapter.
...
These adapters have a stable local skills directory that Stacy can read and manage.
...
These adapters do not have a meaningful Stacy-owned persistent install state.
...
- desired skills stored in Stacy
...
- runtime already injects Stacy skills into `~/.cursor/skills`
...
- runtime already injects Stacy skills into `~/.gemini/skills`
...
- runtime already injects Stacy skills into `~/.pi/agent/skills`
...
- Stacy can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Stacy skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Stacy may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Stacy skills that are clearly marked as Stacy-managed
...
- Stacy stores desired skills only
...
- avoid destructive wording unless Stacy can prove a skill is Stacy-managed
...
That gets Stacy from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
