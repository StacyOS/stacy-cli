...
This document defines the product and UI plan for skill management in Stacy.
...
- Stacy company import/export can include skills as package content
...
3. The UI must always tell the truth about what Stacy knows:
   - desired state in Stacy
...
Stacy should treat skills at two scopes:
...
- created locally inside Stacy later
...
7. “Understand what Stacy knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Stacy-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Stacy did not install.

These should be clearly marked:

- external
- not managed by Stacy
...
- `desired_only`: Stacy wants it, adapter does not show it yet
- `external`: adapter has it but Stacy does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Stacy only tracks desired state
...
- Stacy can still track desired skills
...
- `.stacy.yaml` may add Stacy-specific fidelity, but should not replace the base shortname association model
...
- `.stacy.yaml` may add Stacy-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Stacy should have a company-scoped skill model or managed package model representing:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Stacy-specific.
...
That gives Stacy one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
