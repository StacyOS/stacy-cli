...
Stacy's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Stacy's API, UI, and data model | Yes |
...
modules/
  observability/
    stacy.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@stacy/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@stacy/core";
...
```
1. Scan modules/ directory for stacy.module.json manifests
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@stacy/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@stacy/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:stacy/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.stacy.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm stacy store list                    # browse available modules and templates
pnpm stacy store install <module-id>     # install a module
pnpm stacy store import <template-id>    # import a company template
pnpm stacy store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Stacy instance | `server:started` |
...
Add to `@arpanstacy/stacy-server`:
...
4. **Module config** — `stacy.config.json` with per-module config, env var interpolation
...
10. **`@stacy/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
