...
`PAP-447` asks how Stacy should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Stacy instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Stacy:

- some operators use Stacy to manage Stacy and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Stacy
- Claude and Codex expose different built-in affordances, so Stacy should not overfit to one tool

## Core Product Decision

Stacy should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Stacy
...
- Stacy should not assume that every adapter can see or use a host filesystem path directly
...
If Stacy makes worktrees agent-first, it will blur:
...
- Stacy core resolves and records execution workspace state
...
That gives Stacy room to support:
...
- Stacy handles workspace runtime services under the hood using project/workspace policy
...
Once Stacy is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Stacy cannot infer
...
- when issue moves to done, should Stacy auto-commit?
- should it auto-push?
...
Stacy already has the right foundation for a project-first model.
...
For server-spawned local adapters, Stacy already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Stacy should mirror directly

Implication:

- for `codex_local`, Stacy should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Stacy should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Stacy should separate:
...
Stacy should be able to express intentions such as:
...
These are not specific to Stacy. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".stacy/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Stacy-specific `devServer` flag.
...
Stacy should distinguish between:
...
Stacy should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Stacy should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Stacy
...
For runtime services specifically, Stacy should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Stacy-only dev-server flags
...
3. Inject resolved execution workspace details into `context.stacyWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Stacy instance can be reused through env/config injection when appropriate
...
- Stacy can decide whether to reuse or start a fresh service deterministically
...
3. Test against Stacy-on-Stacy workflows first.
...
7. Workspace runtime services are modeled generically, not as Stacy-specific dev-server toggles.
