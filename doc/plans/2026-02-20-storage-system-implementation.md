...
Add a single storage subsystem for Stacy that supports:
...
- API serves attachment bytes through authenticated Stacy endpoints.
...
- Default local path is under instance root: `~/.stacy/instances/<instanceId>/data/storage`.
...
- `stacy onboard` writes a valid `storage` config block by default.
- `stacy configure --section storage` can switch between local and s3 modes.
...
- `stacy doctor` reports actionable storage status.
