...
- New config/env settings:
  - `STACY_AGENT_JWT_SECRET`
  - `STACY_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `STACY_AGENT_JWT_ISSUER` (default: `stacy`)
  - `STACY_AGENT_JWT_AUDIENCE` (default: `stacy-api`)
...
1. In:
   ...
   inject `STACY_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `STACY_API_KEY`, do not overwrite it.
- Continue injecting:
  - `STACY_AGENT_ID`
  - `STACY_COMPANY_ID`
  - `STACY_API_URL`
...
- Local adapters authenticate without manual `STACY_API_KEY` config.
