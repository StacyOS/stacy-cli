...
This document defines the V1 design for a Stacy adapter that integrates with
...
4. synthesized stdout events for Stacy UI/CLI
...
3. **Agent callback auth to Stacy** uses a bootstrap exchange flow (no long-lived Stacy key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Stacy endpoints, not full SKILL.md prompt injection.
...
- `stacyPublicUrl` (optional override; else `STACY_PUBLIC_URL` env)
...
## Stacy Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `STACY_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.stacyPublicUrl`
2. `process.env.STACY_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Stacy credentials in prompt text.

Flow:

1. Before launch/follow-up, Stacy mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Stacy validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Stacy API calls.
...
1. Prompt includes a compact instruction to fetch skills from Stacy.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/stacy`
   - `GET /api/skills/stacy-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <stacyPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Stacy run context.
...
5. `stacyPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Stacy skill endpoints |
| Stacy auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Stacy context.
3. Add richer PR metadata surfacing in Stacy UI.
