...
This document defines a V1 plan for "smart model routing" in Stacy.
...
- preserve Stacy's existing task, session, and audit invariants
...
That separation is a better fit for Stacy than copying Hermes' exact keyword heuristic.

## 3. Current Stacy State

Stacy already has the right execution shape for adapter-specific routing, but it currently assumes one model per heartbeat run.

Current implementation facts:

- `server/src/services/heartbeat.ts` builds rich run context, including `stacyWake`, workspace metadata, and session handoff context
...
Stacy should implement smart model routing as an adapter-local, opt-in execution pattern.

V1 decision:

1. Do not add a global server-side router that tries to understand every adapter.
2. Do not copy Hermes' prompt-keyword classifier as Stacy's default routing policy.
...
- Stacy heartbeats are structured, issue-scoped, and already include wake metadata
...
- the adapter already injects Stacy skills and workspace metadata cleanly
...
This matters because Stacy's board UI is supposed to make cost and behavior legible.

## 9. Why Not Copy Hermes Exactly

Hermes' cheap-route heuristic is useful precedent, but Stacy should not start there.

Reasons:

- Hermes is optimizing free-form conversational turns
- Stacy agents run structured, issue-scoped heartbeats with explicit task and workspace context
- Stacy already knows whether a run is fresh vs resumed, issue-scoped vs approval follow-up, and what workspace/session exists
- those execution facts are stronger routing signals than prompt keyword matching

If Stacy later wants a cheap-only completion path for trivial runs, that can be a second-stage feature built on observed run data, not the first implementation.
...
Stacy should ship smart model routing as:
