...
This document defines the next-stage plan for Stacy company import/export.

The core shift is:

- move from a Stacy-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Stacy.
...
Stacy already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Stacy
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `stacy.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.stacy.yaml` sidecar for high-fidelity Stacy-specific details
...
11. adapter-aware skill sync surfaces so Stacy can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Stacy at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Stacy sidecar.
...
Stacy must not redefine `SKILL.md`.
...
- Stacy-specific fidelity lives in `.stacy.yaml`
- Stacy may resolve and install `SKILL.md` packages, but it must not require a Stacy-only skill format
...
### 5.4 Base Package Vs Stacy Extension
...
- base package:
  ...
  - no Stacy-only runtime fields by default
- Stacy extension:
  - `.stacy.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Stacy tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`stacy.manifest.json` is not part of the future package direction.
...
Stacy import/export should support these entity kinds:
...
- Stacy can treat “this set of agents came from team package X” as the imported-team model
...
Stacy should support source references in package metadata with:
...
Stacy should define a new adapter surface area around skills:
...
Planned Stacy behavior:

- if an adapter supports read, Stacy should show current skills in the UI
- if an adapter supports write, Stacy should let the user enable/disable imported skills
- if an adapter supports sync, Stacy should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.stacy.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.stacy.yaml` alongside the base package
...
Stacy should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `stacy company export <company-id> --out <path>`
- `stacy company import <path-or-url> --dry-run`
- `stacy company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Stacy storage, or only referenced at import time?
...
3. Should Stacy support direct local folder selection in the web UI, or keep that CLI-only initially?
