...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Stacy plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Stacy.
...
- package: `@arpanstacy/stacy-plugin-kitchen-sink-example`
- plugin id: `stacy.kitchen-sink-example` or `stacy-kitchen-sink-example`

Recommendation: use `stacy-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Stacy event such as `issue.created`
...
Recommendation: default to a Stacy-local endpoint or a stable public echo endpoint to avoid flaky docs.
