...
Agents need API keys to authenticate with Stacy. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Stacy
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Stacy generates a token and passes it directly to the agent
...
`STACY_API_URL`, `STACY_AGENT_ID`, `STACY_COMPANY_ID`. We need to
add a `STACY_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Stacy CLI
...
```
stacy auth login
# Opens browser -> user confirms -> token stored at ~/.stacy/credentials
```
...
managed by the Stacy server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Stacy.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Stacy SKILL.md (or a link to it)
   - What information Stacy needs from the agent (e.g. webhook URL, adapter
...
5. Stacy stores the pending registration.
...
7. On approval, Stacy provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Stacy issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.stacy.ing/skills/stacy/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Stacy should send you heartbeats",
...
1. Generate a Stacy invite link for the company.
...
4. A Stacy company member approves the new agent.
5. Stacy begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `STACY_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `stacy auth login` for developer-managed remote agents.                                      |
