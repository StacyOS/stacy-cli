...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/stacy-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `STACY_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `STACY_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Stacy may create host-local worktrees, processes, and paths
- `cloud`
  - Stacy should assume no durable host-local execution workspace management
...
- use `STACY_EXECUTION_TOPOLOGY` as a defaulting guardrail
