...
Stacy should not replace its workspace runtime service model with VS Code tasks.
...
- Stacy owns long-running workspace services and their desired state
- Stacy shows operators exactly which named thing they are starting or stopping
- Stacy distinguishes long-running services from one-shot jobs
...
This is directionally correct for Stacy because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Stacy does not yet leverage existing workspace metadata.

## Recommendation

Keep Stacy runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Stacy.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Stacy service or job semantics should become runnable in Stacy.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Stacy runtime services.
...
That creates a bad fit if Stacy tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Stacy's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Stacy should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Stacy can resolve
...
- selected variable substitution when Stacy can resolve it safely from workspace context
- mapping task metadata into Stacy readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Stacy cannot execute directly
...
Stacy needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Stacy metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Stacy
...
For a workspace with `cwd`, Stacy should look for `.vscode/tasks.json`.
...
Stacy should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Stacy metadata marks it as a service, or
...
Stacy-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Stacy would import and why.
...
Goal:
run supported imported service commands through the current Stacy supervisor.
...
Success condition:
imported service commands behave like native Stacy services once adopted.
...
- removal of Stacy's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Stacy to VS Code and making the model worse for non-VS-Code repos
...
Stacy should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Stacy workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
