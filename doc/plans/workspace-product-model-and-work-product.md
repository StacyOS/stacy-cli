...
Stacy needs to support two very different but equally valid ways of working:
...
Today, Stacy already has the beginnings of this model:
...
- making Stacy feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Stacy users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Stacy deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Stacy into a full code review product
...
This object must be recorded explicitly so that Stacy can:
...
Stacy should treat PRs as a type of work product linked back to:
...
Stacy already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Stacy must be a first-class deployment mode

Stacy cannot assume that it is running on the same machine as the code.

In cloud deployments, Stacy may:
...
- `Runtime service`: a process or service Stacy owns or tracks for a workspace
...
It also lets cloud-hosted Stacy deployments point at codebases and remotes without pretending the Stacy host has direct filesystem access.
...
Without an explicit `execution workspace` record, Stacy has nowhere stable to attach:
...
Stacy needs a single place to show:
...
- Stacy-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Stacy owning the execution host
...
## 1. Stacy host and execution host must be decoupled
...
without the Stacy host itself running local git or local preview processes.
...
Stacy should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Stacy.
...
Stacy should keep workspace automation centered on a small set of hooks:
...
Stacy should provide the equivalent visibility in the project `Code` surface:
...
Stacy should treat execution workspaces as the natural home for:
...
Stacy should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Stacy should continue treating cleanup policy as part of the core workspace model:
...
Stacy should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Stacy host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Stacy should borrow Worktrunk's good execution patterns while keeping the broader Stacy model:
...
That keeps Stacy focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Stacy deployments remain first-class

That is a better fit for Stacy than either extreme:
