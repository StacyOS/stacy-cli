...
ClipHub is the public registry where people share, discover, and download Stacy company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Stacy what a package registry is to a programming language. Stacy already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Stacy — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Stacy instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Stacy spec. It contains:
...
**Install (fresh start):**
```
stacy install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Stacy instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Stacy, export your company as a template, then publish:

```
stacy export --template my-company
stacy publish cliphub my-company
```
...
ClipHub is a **separate service** from Stacy itself. Stacy is self-hosted; ClipHub is a hosted registry that Stacy instances talk to.
...
| **Stacy CLI** | `stacy install`, `stacy publish`, `stacy cliphub sync` — built into Stacy |
| **Stacy UI** | "Browse ClipHub" panel in the Stacy web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Stacy) |
| Backend | TypeScript + Hono (consistent with Stacy) |
| Database | PostgreSQL |
...
4. Run `stacy install cliphub:acme/lean-saas-shop`
5. Stacy creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Stacy until it works well
2. Export: `stacy export --template my-agency`
3. Publish: `stacy publish cliphub my-agency`
...
3. Install just that agent: `stacy install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Stacy

ClipHub is **not required** to use Stacy. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Stacy what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `stacy install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Stacy web UI
- [ ] `stacy cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Stacy's own philosophy)
