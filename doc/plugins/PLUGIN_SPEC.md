# Stacy Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Stacy's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Stacy app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Stacy HTTP APIs directly.
...
Stacy plugin design is based on the following assumptions:

1. Stacy is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Stacy business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Stacy core.
...
2. Let plugins add major capabilities without editing Stacy core.
...
4. Depend on project-local plugin folders such as `.stacy/plugins`.
...
The single Stacy deployment an operator installs and controls.

### 5.2 Company

A first-class Stacy business object inside the instance.
...
A trusted in-process extension loaded directly by Stacy core.
...
An installable instance-wide extension package loaded through the Stacy plugin runtime.
...
Stacy has two extension classes.
...
Stacy already has a concrete workspace model:
...
Plugins live under the Stacy instance directory.

Suggested layout:

- `~/.stacy/instances/default/plugins/package.json`
- `~/.stacy/instances/default/plugins/node_modules/`
- `~/.stacy/instances/default/plugins/.cache/`
- `~/.stacy/instances/default/data/plugins/<plugin-id>/`
...
Stacy should add CLI commands:

- `pnpm stacy plugin list`
- `pnpm stacy plugin install <package[@version]>`
- `pnpm stacy plugin uninstall <plugin-id>`
- `pnpm stacy plugin upgrade <plugin-id> [version]`
- `pnpm stacy plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@stacy/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface StacyPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Stacy agents can use during runs.
...
- Stacy server starts one worker process per installed plugin
...
Receives one typed Stacy domain event.
...
export function definePlugin(definition: PluginDefinition): StacyPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: StacyPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@arpanstacy/stacy-plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@arpanstacy/stacy-plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@arpanstacy/stacy-plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@arpanstacy/stacy-plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@arpanstacy/stacy-plugin-sdk/ui`
...
The `@arpanstacy/stacy-plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Stacy secret provider system rather than a plain text input
...
1. Core Stacy data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Stacy objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Stacy product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Stacy later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Stacy secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm stacy plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Stacy server. This is a normative requirement, not optional.
...
### 27.1 `@stacy/plugin-test-harness`
...
```ts
import { createTestHarness } from "@stacy/plugin-test-harness";
...
For developing a plugin against a running Stacy instance:

- The operator installs the plugin from a local path: `pnpm stacy plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-stacy-plugin`) that scaffolds:

- `package.json` with correct `stacyPlugin` keys
...
- `@stacy/plugin-workspace-files`
- `@stacy/plugin-terminal`
- `@stacy/plugin-git`
- `@stacy/plugin-linear`
- `@stacy/plugin-github-issues`
- `@stacy/plugin-grafana`
- `@stacy/plugin-runtime-processes`
- `@stacy/plugin-stripe`
...
- `@arpanstacy/stacy-plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@arpanstacy/stacy-plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@arpanstacy/stacy-plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@arpanstacy/stacy-plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@arpanstacy/stacy-plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@arpanstacy/stacy-plugin-sdk/ui`
...
- `@stacy/plugin-test-harness`
- `create-stacy-plugin` starter template
...
Stacy should not implement a generic in-process hook bag modeled directly after local coding tools.

Stacy should implement:
...
That is the complete target design for the Stacy plugin system.
