...
For `claude_local` and `codex_local`, Stacy assumes the CLI is already installed and authenticated on the host machine.
...
Stacy stores resumable session state per `(agent, taskKey, adapterType)`.
