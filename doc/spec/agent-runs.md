...
This spec defines how Stacy actually runs agents while staying runtime-agnostic.
...
1. Stacy is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Stacy manages lifecycle, and on exit Stacy parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `stacy.skill` (shared Stacy skill text block)
...
2. Preferred transport is env vars (`STACY_*`) injected at runtime.
